...
function HistoryPage() {
  ...
  return (
    <SiteLayout>
      <PageIntro
        label="History"
        title="Kashmir's past, with the evidence in view"
        description="This timeline follows the valley from its earliest settlements to 1947. Labels show where an account rests on archaeology, where scholars disagree, and where it comes from inherited tradition."
      />

      <section aria-labelledby="evidence-key" className="mx-auto max-w-5xl px-4 pb-10">
        <div className="flex max-w-3xl flex-col gap-4 border-y border-border py-6">
          <SectionHeading
            id="evidence-key"
            title="How to read the evidence"
            description="Archaeology can test dates, settlements, and objects. Literary works preserve memory, belief, and argument, but are not simple transcripts of events. The Nilamata Purana and the Rajatarangini are essential sources whose genre and date must remain visible."
          />
          <div className="flex flex-wrap gap-2" aria-label="Evidence status key">
      ...
      <section aria-label="Historical timeline" className="mx-auto max-w-4xl px-4 pb-16">
        <ol className="relative ms-2 flex flex-col gap-6 border-s-2 border-primary/30 sm:ms-4 sm:gap-8">
          {timeline.map((entry) => (
            <li key={entry.id} className="relative ps-5 sm:ps-7">
              <span
                aria-hidden="true"
                className="absolute -start-[9px] top-6 size-4 rounded-full border-2 border-background bg-primary"
              />
              <button
                type="button"
                onClick={() => setActive(entry)}
                className="w-full rounded-xl border border-border bg-card p-5 text-left shadow-sm transition-[background-color,border-color,box-shadow] duration-150 hover:border-primary/60 hover:shadow-[var(--surface-shadow)] active:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background sm:p-6"
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <h2 className="font-serif text-xl text-foreground sm:text-2xl">{entry.era}</h2>
                  <span className="tabular-nums rounded-full bg-primary/10 px-2.5 py-1 text-xs font-semibold text-primary">
                    {entry.years}
                  </span>
                </div>
                {entry.status && (
          ...
          <li className="relative ps-5 sm:ps-7">
            <span
              aria-hidden="true"
              className="absolute -start-[9px] top-6 size-4 rounded-full border-2 border-background bg-muted-foreground/40"
            />
            <div className="border-y border-dashed border-border py-5">
              <h2 className="font-serif text-xl text-foreground">Modern history (after 1947)</h2>
      ...
      <section aria-labelledby="myths-heading" className="border-y border-border bg-muted/40">
        <div className="mx-auto max-w-5xl px-4 py-14 sm:py-16">
          <SectionHeading
            id="myths-heading"
            title="Common myths, checked"
            description="Some familiar stories come from tradition rather than firm historical evidence. Here is what the available research can and cannot tell us about them."
          />
          <div className="mt-8 grid md:grid-cols-2 md:gap-x-8">
            {historyMyths.map((item) => (
              <article key={item.myth} className="border-t border-border py-6">
                <EvidenceBadge status={item.status} />
                <h3 className="mt-4 font-serif text-xl text-foreground">Myth: {item.myth}</h3>
                <p className="mt-3 text-sm leading-relaxed text-muted-foreground sm:text-base">
          ...
          {active && (
            <>
              ...
              <div className="flex flex-col gap-4 text-sm">
                <p className="font-semibold leading-relaxed text-foreground">{active.summary}</p>
