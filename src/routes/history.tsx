  ...
  return (
    <SiteLayout>
      <section className="mx-auto max-w-3xl px-4 pt-12 pb-6">
        <p className="text-sm uppercase tracking-[0.2em] text-primary font-semibold">History</p>
        <h1 className="mt-3 font-serif text-4xl text-foreground">A short history of Kashmir</h1>
        <p className="mt-4 text-muted-foreground leading-relaxed">
          From the old story of a drained lake to the end of Dogra rule in 1947 — a calm walk
          through the eras of Kashmir. Tap any entry to read more.
        </p>
      </section>
          ...
          {timeline.map((entry) => (
            <li key={entry.id} className="ps-6 relative">
              <span
                ...
                style={{
                  background: "linear-gradient(135deg, var(--saffron), var(--chinar))",
                }}
              />
              <button
                type="button"
              ...
              >
                ...
                <p className="mt-2 text-sm text-muted-foreground leading-relaxed">{entry.fact}</p>
          ...
          <li className="ps-6 relative">
            ...
            <div className="rounded-xl border border-dashed border-border bg-muted/40 p-5">
              ...
              <p className="mt-2 text-sm text-muted-foreground leading-relaxed">
                Modern Kashmir history is beyond the scope of this site for now. We focus on
                education, not politics or news.
              </p>
            </div>
          </li>
        </ol>
          ...
          {active && (
            <>
              ...
              <div className="space-y-4 text-sm">
                <p className="text-foreground font-medium leading-relaxed">{active.summary}</p>
                ...
                {active.developments.length > 0 && (
                  <div>
                    <h3 className="font-serif text-base text-foreground mb-1">In this age</h3>
                ...
                {active.spotlight && (
                  ...
                  >
                    ...
                    <p className="text-foreground/90 leading-relaxed">{active.spotlight.body}</p>
                  </div>
                )}
                {active.people.length > 0 && (
                  <div>
                    <h3 className="font-serif text-base text-foreground mb-1">Notable people</h3>
                    <ul className="list-disc pl-5 text-muted-foreground space-y-0.5">
                      {active.people.map((p) => (
                        <li key={p.name}>
